...
__date__ = "2009-08-15 -- 2012-09-05"
...
{{
  ...
  if (!(PyArray_ISCONTIGUOUS(xa) && PyArray_TYPE(xa) == NPY_DOUBLE))
    SWIG_exception(SWIG_TypeError, "Contigous numpy array of doubles expected. Make sure the numpy array is contiguous, and uses dtype=float_.");

  // Check size of passed array
  if ( PyArray_SIZE(xa) != {num_states} )
    SWIG_exception(SWIG_ValueError, "Expected a numpy array of size: {num_states}, for the dy argument.");
...
{{
  ...
  // Check size of passed array
  if ( PyArray_SIZE(xa) != {num_states} )
    SWIG_exception(SWIG_ValueError, "Expected a numpy array of size: {num_states}, for the states argument.");
...
{{
  ...
  if ( PyArray_SIZE(xa) != {num_parameters} )
    SWIG_exception(SWIG_ValueError, "Expected a numpy array of size: {num_parameters}, for the parameters argument.");
...
def compile_extension_module(code, ode):
    ...
    module_name = "gotran_compiled_module_{0}_{1}".format(ode.name, hashlib.md5(\
        repr(code) + instant.get_swig_version() + gotran2.__version__).hexdigest())
