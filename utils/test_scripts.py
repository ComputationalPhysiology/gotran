...
cmd2file = {'gotran2py': ['tentusscher_2004_mcell_updated.py'],
            ...
            'gotran2matlab': ['tentusscher_2004_mcell_updated_init_parameters.m',
    ...
    if command in cmd2file:
        for f in cmd2file[command]:
            if not os.path.isfile(f):
                failed = 1
                msg = ('Command {} failed: '
                       'File {} does not exist.').format(command, f)
                failed_output.append(msg)
            else:
                os.remove(f)

end = time.time()
print('Ran {} tests in {:.2f} s'.format(num_tests, end-start))
if failed:
