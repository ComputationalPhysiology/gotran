...
class TestBase(object):
    def test_run(self):
        ...
        rel_diff = np.abs((ref_interp_data - comp_data) / data_range).sum() / len(
            comp_data
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        not_supported = [
            not_sup + "\\\\{{\\it {0}}}".format(skip.get(not_sup, "").capitalize())
            for not_sup in not_supported
        ]

        supported_str = "\n".join(
            "\item {0}".format(" ".join(part.capitalize() for part in model.split("_")))
        ...
        # print supported_str
        open("supported_cellml_models.tex", "w").write(
            supported_models_form.format(supported_str, not_supported_str)
        )
