...
def get_dtype_str(float_precision):
    return {False: 'float32',
            True: 'float64'}[float_precision]

def get_float_prec_str(float_precision):
    return {False: 'single',
            True: 'double'}[float_precision]
...
def get_store_field_states_fn(num_nodes):
    ...
    def store_field_states_fn(field_states):
        stored_field_states[i].append([
            field_states[0],
            field_states[num_nodes/2],
            field_states[num_nodes-1]])
...
def get_g_to_field_parameter_values(num_nodes, float_precision):
    return \
        0.294*np.arange(0,
                        num_nodes,
                        dtype=get_dtype_str(float_precision)
        )[::-1]/(num_nodes-1.0)
...
class TestCase(object):
    def __init__(self,
        ...
        self.field_states_fn = field_states_getter_fn(num_nodes) \
                               if field_states_getter_fn is not None else None
        self.field_parameters = field_parameters
        self.field_parameter_values = \
            field_parameter_values_getter_fn(num_nodes, double) \
            if field_parameter_values_getter_fn is not None else None
...
def createTestCases(ode,
                    ...
                    update_field_states=(True,)):
    testcases = list()
    for comb in it.product(ode, num_nodes, dt, tstop, t0, solver, field_states,
                           field_states_getter_fn, field_parameters,
                           field_parameter_values_getter_fn, block_size, double,
                           statesrepr, paramrepr, bodyrepr, use_cse, update_host_states,
                           update_field_states):
...
def testSolvers(num_nodes=1024, dt=0.001, tstop=50.0,
...
def testDt(num_nodes=1024*8, dt=(1, 0.5, 0.2, 0.1, 0.05), tstop=300.0, solver='rush_larsen',
           field_states=['V'],
           field_states_getter_fn=get_store_field_states_fn,
           field_parameters=['g_to'],
           field_parameter_values_getter_fn=get_g_to_field_parameter_values):
...
def testUpdateStates(num_nodes=1024*8, dt=0.1, tstop=300.0, solver='rush_larsen',
    ...
    names = ['host={0}, field={1}'.format(h, f)
...
def testRepresentation(num_nodes=1024*16, dt=0.1, tstop=300.0, solver='rush_larsen',
    ...
    names = ['s={0}, p={1}, b={2}, cse={3}'.format(s, p, b, cse) for s, p, b, cse
             in it.product(statesrepr, paramrepr, bodyrepr, use_cse)]
...
def printResults(names, all_field_states, runtimes, indent=0):
    for name, fstates, runtime in zip(names, all_field_states, runtimes):
        print ' '*indent+'{0:{1}s}: {2:{3}.2f}s ({4}, {5}, {6})'.format(
            ...
            runtime, max(map(len, map(str, map(int, runtimes)))) + 3,
