...
class COSSTestCase(object):
    def __init__(self, **kwargs):
        ...
        if 'field_states' in params \
                and isinstance(params['field_states'], dict):
            params['field_states'] = params['field_states'][params['ode_model']]

        if 'field_parameters' in params \
                and isinstance(params['field_parameters'], dict):
            params['field_parameters'] = \
                params['field_parameters'][params['ode_model']]
        ...
        if self.field_states_getter_fn is None:
            ...
            self.field_states_fn = \
                self.field_states_getter_fn(self.num_nodes,
                                            self.stored_field_states)
        initial_field_params = [param.init for param in self.ode.parameters
        ...
        if self.field_parameter_values_getter_fn is None:
            ...
            self.field_parameter_values = \
                self.field_parameter_values_getter_fn(
                    initial_field_params, self.num_nodes, self.double)

    @staticmethod
    def default_parameters():
        return {'ode_model': None,
                'num_nodes': None,
                'dt': None,
                'tstop': None,
                't0': 0.0,
    ...
    def parameter_iterability_map():
        return {k: v == [""]
                for k, v in COSSTestCase.default_parameters().items()}

    @staticmethod
    def check_kwargs(**kwargs):
        for key in COSSTestCase.required_parameters():
            if key not in kwargs:
                raise ValueError('Missing required argument \'{0}\''.format(
                    key))
...
def dict_product(dicts):
    return (dict(it.izip(dicts, prod))
            for prod in it.product(*dicts.itervalues()))
...
def createTestCases(**kwargs):
    ...
    for key, value in params.iteritems():
        if param_iterability_map[key] and isinstance(value, (list, tuple)) \
...
def runTestSuites(test_suites=None, keep_cuda_code=False, saveDirectory=None,
    ...
    for title, test_suite in test_suites.iteritems():
        results[title] = runTestSuite(title,
...
def printResults(results, indent=0):
    ...
    for result in results:
        name, runtime, fstates, error = \
            result['name'], result['runtime'], result['field_states'], \
            result['error']
        fstates_fmt = '{0}, {1}, {2}'.format(
                fstates[0], fstates[len(fstates)/2], fstates[-1]) \
            if fstates is not None else 'No field states'
        error_fmt = '(ERROR)' if error is not None else ''
        print ' '*indent+'{0:{1}s}: {2:{3}.2f}s ({4}) {5}'.format(
            name, max_name_len, runtime, max_runtime_str_len, fstates_fmt,
            error_fmt)
    print
...
def linear_field_parameter_transform(field_parameters, num_nodes, float_precision):
    if len(field_parameters) == 0:
        ...
        return np.concatenate(tuple(np.reshape(np.linspace(
            value, value*.01, num_nodes), (num_nodes, 1))
                                    for value in field_parameters),
                              axis=1).astype(getFloatPrecisionDtypeStr(
                                                 float_precision)).copy()
...
sample_test_suites = {
    ...
    'ODE MODELS': {
        'ode_model': (ODE_MODELS['tentusscher'],
                      ODE_MODELS['beeler'],
                      ODE_MODELS['grandi']),
        'num_nodes': 8192,
        'dt': 0.1,
...
def testEverything(subset=None, keep_cuda_code=False, printTimings=True,
    ...
    return runTestSuites(test_suites=test_suites, keep_cuda_code=keep_cuda_code,
                         printTimings=printTimings)
...
class ThreadedTest(threading.Thread):
    ...
    def run(self):
        ...
        self.results[self.threadID] = runTestCase(
            self.testcase, 'Thread {0}'.format(self.threadID),
            keep_cuda_code=False, printTimings=False, solver=self.solver)
...
def testSimultaneous(n=4, **kwargs):
    ...
    for threadID, testcase in enumerate(testcases):
        ...
        solver = coss.CUDAODESystemSolver(
            ...
            init_field_parameters=testcase.field_parameter_values,
            params=params)
        threads.append(ThreadedTest(threadID, testcase, solver,
                                    results, threadLock))
    ...
    with open(_file) as f:
        ...
        for _ in xrange(n):
            ...
            datum['field_parameter_values'] = \
                [map(float, fpv) for fpv
                 in map(ast.literal_eval,
                        ['[' + s.replace(']', ' ').replace('[', ' ') + ']' for s
                         in f.readline().rstrip().split('] [')])]
            ...
            if get_stored_fstates:
                for _ in xrange(datum['n_stored_fstate_iters']):
                    datum['stored_field_states'].append(map(float, f.readline().rstrip().split(' ')))
            else:
                for _ in xrange(datum['n_stored_fstate_iters']):
                    f.readline()
